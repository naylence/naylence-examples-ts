...
import "@naylence/advanced-security";
...
function captureInputState(): InputState {
  return {
    ...
    encryptionLevel: getElementById<HTMLSelectElement>("encryptionLevel").value,
  };
}
...
function restoreInputState(): boolean {
  ...
  try {
    ...
    if (typeof parsed.encryptionLevel === "string") {
      getElementById<HTMLSelectElement>("encryptionLevel").value = parsed.encryptionLevel;
    }
    return true;
  } catch {
    storage.removeItem(INPUT_STATE_STORAGE_KEY);
...
async function runOverlayDemo(options: RunOptions = {}): Promise<void> {
  ...
  try {
    ...
    const encryptionLevel = getElementById<HTMLSelectElement>("encryptionLevel").value;
    inputState.encryptionLevel = encryptionLevel;

    // Apply the selected encryption level by updating the environment
    if (typeof window !== "undefined") {
      const envVars = (window as Window & { __ENV__?: Record<string, string | undefined> }).__ENV__;
      if (envVars) {
        envVars.FAME_DEFAULT_ENCRYPTION_LEVEL = encryptionLevel;
      }
    }

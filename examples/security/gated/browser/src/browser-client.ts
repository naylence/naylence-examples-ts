...
const INPUT_STATE_STORAGE_KEY = `naylence.gated.inputs.${ENV_VARS.FAME_ADMISSION_CLIENT_ID ?? "anonymous"}`;
...
async function runGatedDemo(options: RunOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (options.resume) {
      ...
      setStatus("Connecting to fabric with gated security…", "loading");
    }
    ...
    clearInputState();
    setStatus("✔ Gated-secured RPC and streaming completed.", "success");
  } catch (error) {
    ...
    console.error("Gated browser client failed:", error);
  } finally {
    runInFlight = false;
...
function main(): void {
  try {
    ...
    if (shouldAutoResumePkce()) {
      void runGatedDemo({ resume: true });
    }
  } catch (error) {
    console.error("Failed to initialise gated browser client:", error);
    setStatus("✗ Failed to initialise gated client. See console for details.", "error");
  }
}
